...
namespace AVA.UI.CORE.UPS.Sessions
{
    ...
    public class SessionManager
    {
        ...
        // ── Legacy IModelEntry support ────────────────────────────────────────

        /// <summary>
        /// Flat list of all available model entries.
        /// Used for backwards compatibility with existing UI components.
        /// </summary>
        public IEnumerable<IModelEntry> AvailableModelEntries =>
            _Client.GetModelEntries();
        ...
        /// For Nomi: loads the roster and merges discovered models.
            ...
            CancellationToken Token = default)
        {
            ...
            if (IsNomi)
            {
                await _Client.InitializeNomiAsync(Profile.ApiKey, Token);

                // Merge API-discovered models
                var Roster = _Client.GetNomiRoster();
                if (Roster != null)
                {
                    var Discovered = Roster.GetModelEntries()
                        .Select(E => new ModelProfile
                        {
                            Id = E.Id,
                            Label = E.Label,
                            Type = E.Type,
        ...
        /// <summary>
        /// Initializes Nomi connectivity directly from API key.
        ...
        public async Task InitializeNomiAsync(
            ...
            CancellationToken Token = default)
        {
            await _Client.InitializeNomiAsync(ApiKey, Token);
            Notify();
        }
            ...
            string Method = "chat")
        {
            ...
            var Session = new UPSSession(Entry, _Client, Target, Method);
        ...
        /// <summary>
        /// Creates a session from a raw IModelEntry.
        /// Used for backwards compatibility and legacy code paths.
        /// </summary>
        public UPSSession CreateSession(
            ...
            string Method = "chat")
        {
            var Session = new UPSSession(ModelEntry, _Client, Target, Method);
        ...
        /// <summary>
        /// Creates sessions for all available models.
        /// Used for full broadcast topology.
        /// </summary>
        public IEnumerable<UPSSession> CreateAllSessions(
            string Target = "Nomi",
            string Method = "chat")
        {
            var Created = new List<UPSSession>();

            foreach (var Model in AvailableModelEntries)
            {
                var Session = CreateSession(Model, Target, Method);
                Created.Add(Session);
            }
