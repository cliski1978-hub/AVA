...
namespace AVA.UI.CORE.Services
{
    ...
    public sealed class StatePersistenceVerifier
    {
        ...
        public StatePersistenceVerifier(AvaSettingsService settingsService)
        {
        ...
        {
            var originalVaults = CloneVaults(_settingsService.Vaults);
            ...
            try
            {
                ...
                await _settingsService.SaveVaultsAsync(new List<VaultState> { sample });
                var reloaded = await _settingsService.LoadVaultsAsync();
                ...
                return new StateVerificationResult
                {
                    Success = ok,
                    Message = ok
                        ? "State models round-trip and settings persistence verified."
                        : "State persistence reload did not match expected structure."
                };
            }
            finally
            {
                await _settingsService.SaveVaultsAsync(originalVaults);
            }
        }
