...
namespace AVA.UI.CORE.UPS.Client
{
    ...
    public class UPSClientService
    {
        ...
        // Nomi uses the UPS gateway path; everything else uses ILLMAdapter.
        ...
        /// <summary>
        /// Registers all models within a non-Nomi LLM profile.
        ...
        /// Nomi: routes through the UPS gateway (NomiProtocolAdapter).
            ...
            CancellationToken Token = default)
        {
            ...
            // ── Non-Nomi: use ILLMAdapter directly ────────────────────────────
            if (!isNomi && model != null)
            {
                var key = AdapterKey(profile.ProfileId, model.Id);
            ...
            // ── Nomi: route through UPS gateway ───────────────────────────────
            ...
            var responseEnvelope = await SendAsync(
                "Nomi", "chat", uparams, Token: Token).ConfigureAwait(false);

            if (responseEnvelope == null)
                return new UPSResponse { Success = false, ErrorMessage = "No response from Nomi." };
