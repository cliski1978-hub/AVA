...
{
    public sealed class VaultWorkflowLineStepNotesReadService : IVaultWorkflowLineStepNotesReadService
    {
        ...
        public VaultWorkflowLineStepNotesReadService(
        ...
        public async Task<VaultAttachedNotesResponse> GetNotesForWorkflowLineStepAsync(string workflowLineStepId, CancellationToken ct = default)
        {
            ...
            var links = await _lineStepNoteQuery.GetByWorkflowLineStepIdAsync(workflowLineStepId, ct);
            return await BuildResponseAsync(links, workflowLineStepId, "WorkflowLineStep", null, ct);
        }

        public async Task<VaultAttachedNotesResponse> GetRequiredNotesForWorkflowLineStepAsync(string workflowLineStepId, CancellationToken ct = default)
        {
            if (string.IsNullOrWhiteSpace(workflowLineStepId))
                throw new ArgumentException("WorkflowLineStep ID is required.", nameof(workflowLineStepId));
        ...
        public async Task<VaultAttachedNotesResponse> GetOptionalNotesForWorkflowLineStepAsync(string workflowLineStepId, CancellationToken ct = default)
        {
            if (string.IsNullOrWhiteSpace(workflowLineStepId))
                throw new ArgumentException("WorkflowLineStep ID is required.", nameof(workflowLineStepId));
        ...
        public async Task<VaultAttachedNoteDto?> GetNoteForWorkflowLineStepAsync(string workflowLineStepId, string noteId, CancellationToken ct = default)
        {
            if (string.IsNullOrWhiteSpace(workflowLineStepId))
                throw new ArgumentException("WorkflowLineStep ID is required.", nameof(workflowLineStepId));
            ...
            return MapToDto(link, note, workflowLineStepId, "WorkflowLineStep");
        }

        public async Task<int> CountNotesForWorkflowLineStepAsync(string workflowLineStepId, CancellationToken ct = default)
        {
            if (string.IsNullOrWhiteSpace(workflowLineStepId))
                throw new ArgumentException("WorkflowLineStep ID is required.", nameof(workflowLineStepId));

            return await _lineStepNoteQuery.CountByWorkflowLineStepIdAsync(workflowLineStepId, ct);
        }

        private async Task<VaultAttachedNotesResponse> BuildResponseAsync(
            IEnumerable<VaultWorkflowLineStepNote> links,
            string parentId,
            string parentType,
        ...
        private static VaultAttachedNoteDto MapToDto(
            VaultWorkflowLineStepNote link,
            VaultNote note,
            string parentId,
            string parentType)
        {
